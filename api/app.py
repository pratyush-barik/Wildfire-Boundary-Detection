...
def find_hotspots(image_path):
    ...
    if image is None:
        return None, "Error: Could not read the image file."
...
def analyze_and_plot(hotspot_coords, result_filename):
    ...
    if not hotspot_coords or len(hotspot_coords) < 3:
        return 0, None, "Error: Fewer than 3 hotspots detected."
    ...
    plt.plot(
        ...
        "r-",
        linewidth=2,
        label=f"Wildfire Boundary (Convex Hull)\nArea: {area:.2f} sq units",
    )
    ...
    return area, f"/static/{result_filename}", None
